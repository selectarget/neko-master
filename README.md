...
Neko Master 不仅仅是一个监控面板，现在它内置了 **Mihomo (Clash Meta)** 核心，可以直接作为 **Web 版的代理客户端** 使用（类似 Clash Verge Rev，但是 Web 化的）。

### 功能特性

*   **内置核心管理**：自动下载并管理 Mihomo 核心，无需手动配置。
*   **配置文件管理**：支持通过订阅 URL 添加、更新、删除代理配置文件。
*   **一键切换**：多配置文件快速切换。
*   **系统代理 (实验性)**：支持 Windows / macOS 下一键开启系统代理（需要宿主机运行环境）。
*   **TUN 模式**：支持开启 TUN 模式，实现系统级接管（需要管理员权限）。

### 使用方法

1.  启动 Neko Master。
2.  点击左侧边栏的 **"客户端" (Client)** 菜单。
3.  点击 **"配置" (Configuration)** 标签页。
4.  点击 **"添加配置" (Add Profile)**，输入名称和订阅链接。
5.  添加成功后，点击 **"激活" (Activate)** 切换到该配置。
6.  点击顶部的 **"启动" (Start)** 按钮运行代理核心。

> **注意**：
> *   **Docker 环境**：在 Docker 中运行时，"系统代理" 功能对宿主机无效。TUN 模式需要容器拥有 `NET_ADMIN` 权限。
> *   **源码/本地运行**：在 Windows/macOS 上直接运行源码或构建版本时，支持完整的系统代理控制。
